...
const createProduct = (values, { resetForm }) => {
    axios.post('/type_application_table', values)
        .then((response) => {
            resetForm();
            ...
            Toast.fire({
                icon: 'info',
                title: `Tipo de producto creado correctamente `
            })
        })
        .catch((error) => {
...
const onUpdateproduc = (values, { resetForm }) => {
    axios.put(`/type_application_table/${formProducts.value.id}`, values)
        .then((response) => {
            resetForm();
            modalRef.value.hide('modalActions');
            emit('edit');
...
const onDeleteProduct = (values) => {
    ...
    SwalStyle.fire({
        title: '<h4 class="text-muted fw-bold" >¿Esta seguro de eliminar el tipo de aplicacion?</h4>',
        icon: "error",
        width: "26em",
        ...
        if(result.isConfirmed){
               axios.delete(`/type_application_table/${values.id}`).then(()=>{
                emit('edit');
            var Toast = Swal.mixin({
                toast: true,
                position: 'top-right',
            ...
            Toast.fire({
                icon: 'success',
                title: `Tipo de aplicacion borrado correctamente `
            });
               }) 
               resolve(true)
        }else{
...
<template>
    ...
    <div>
        ...
        centered ref="modalRef" hide-header-close
            :title="edit ? 'Editar' : 'Crear tipo de aplicacion'">
