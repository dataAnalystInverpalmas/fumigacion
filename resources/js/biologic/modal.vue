...
const createProduct = (values, { resetForm }) => {
    axios.post('/biologic_table', values)
        .then((response) => {
            resetForm();
            ...
            Toast.fire({
                icon: 'info',
                title: `Blanco Biologico correctamente `
            })
        })
        .catch((error) => {
...
const onUpdateproduc = (values, { resetForm }) => {
    axios.put(`/biologic_table/${formProducts.value.id}`, values)
        .then((response) => {
            resetForm();
            modalRef.value.hide('modalActions');
            emit('edit');
            ...
            Toast.fire({
                icon: 'success',
                title: `Blanco Biologico actualizado correctamente `
            })
        })
        .catch((error) => {
            if (error.response.data.errors) {
...
const onDeleteProduct = (values) => {
    ...
    SwalStyle.fire({
        ...
        if(result.isConfirmed){
               axios.delete(`/biologic_table/${values.id}`).then(()=>{
                emit('edit');
            var Toast = Swal.mixin({
                toast: true,
                position: 'top-right',
...
<template>
    ...
    <div>
        ...
        centered ref="modalRef" hide-header-close
            :title="edit ? 'Editar' : 'Crear blaco biologico'">
