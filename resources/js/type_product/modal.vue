...
const createProduct = (values, { resetForm }) => {
    axios.post('/type_product_table', values)
        .then((response) => {
            resetForm();
...
const onUpdateproduc = (values, { resetForm }) => {
    axios.put(`/type_product_table/${formProducts.value.id}`, values)
        .then((response) => {
            resetForm();
            modalRef.value.hide('modalActions');
            emit('edit');
...
const onDeleteProduct = (values) => {
    ...
    SwalStyle.fire({
        title: '<h4 class="text-muted fw-bold" >¿Esta seguro de eliminar el tipo de producto?</h4>',
        icon: "error",
        width: "26em",
        ...
        if(result.isConfirmed){
               axios.delete(`/type_product_table/${values.id}`).then(()=>{
                emit('edit');
            var Toast = Swal.mixin({
                toast: true,
                position: 'top-right',
            ...
            Toast.fire({
                icon: 'success',
                title: `Tipo de producto borrado correctamente `
            });
               }) 
               resolve(true)
        }else{
...
<template>
    ...
    <div>
        ...
        centered ref="modalRef" hide-header-close
            :title="edit ? 'Editar' : 'Crear tipo de producto'">
