...
const createProduct = (values, { resetForm }) => {
    axios.post('/unit_meansure', values)
        .then((response) => {
            resetForm();
...
const onUpdateproduc = (values, { resetForm }) => {
    axios.put(`/unit_meansure/${formProducts.value.id}`, values)
        .then((response) => {
            resetForm();
            modalRef.value.hide('modalActions');
            emit('edit');
...
const onDeleteProduct = (values) => {
    ...
    SwalStyle.fire({
        title: '<h4 class="text-muted fw-bold" >¿Esta seguro de eliminar la unidad de medida?</h4>',
        icon: "error",
        width: "26em",
        ...
        if(result.isConfirmed){
               axios.delete(`/unit_meansure/${values.id}`).then(()=>{
                emit('edit');
            var Toast = Swal.mixin({
                toast: true,
                position: 'top-right',
            ...
            Toast.fire({
                icon: 'success',
                title: `Unidad borrada correctamente `
            });
               }) 
               resolve(true)
        }else{
...
<template>
    ...
    <div>
        ...
        centered ref="modalRef" hide-header-close
            :title="edit ? 'Editar' : 'Crear unidad de medida'">
