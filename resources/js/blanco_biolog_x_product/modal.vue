...
const myOptionsBlancoBiolg = ref(null);
...
const createProduct = (values, { resetForm }) => {
    axios.post('/biolog_x_product', values)
        .then((response) => {
            resetForm();
            ...
            Toast.fire({
                icon: 'info',
                title: `Blanco biologico creado correctamente `
            })
        })
        .catch((error) => {
...
const onDeleteProduct = (values) => {
    return new Promise((resolve, reject) => {
        ...
        SwalStyle.fire({
            title: '<h4 class="text-muted fw-bold" >¿Esta seguro de eliminar el Blanco biologico?</h4>',
            icon: "error",
            width: "26em",
            ...
            if (result.isConfirmed) {
                axios.delete(`/biolog_x_product/${values.id_blanco_biolog_prod}`).then(() => {
                    emit('edit');
                    var Toast = Swal.mixin({
                        toast: true,
                        position: 'top-right',
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: `Blanco biologico  borrado correctamente `
                    });
                })
                resolve(true)
            } else {
...
function getBlancoBioliList() {
    axios.get('/biologic_table').then(function (response) {
        ...
        myOptionsBlancoBiolg.value = obj;
...
<template>
    <div tabindex="0">
        ...
        <b-modal :tabindex="1" id="modalActions" :header-class="{
            ...
            hide-header-close :title="edit ? 'Editar' : 'Crear blancos biologicos para el producto'">
            ...
            settingOption: value, settingOption: value
                ...
                <div class="row p-2">
                    <label forv="id_turno" class="col-sm-3 col-form-label text-primary">Blancos Biologicos:</label>
                    <div class="form-group col-sm-8 ">

                        <Field ref="blanco_biol_x_producto" name="blanco_biol_x_producto" as="Select2" v-model="myValue"
                            :options="myOptionsBlancoBiolg" :settings="{
            multiple: 'true',
            theme: 'bootstrap-5',
            width: '100%',
            containerCssClass: 'select2--small',
