...
const columns = ref([
  ...
  { data: 'ingrediente_activo', title: 'Ingrediente activo' },
  {
    data: null,
    render: '#action',
    title: 'Action',
...
function getIngredientActiveProduct() {
  axios.get('/ingredient_activex_product_table').then(function (response) {
...
onMounted(() => {
  getIngredientActiveProduct();
  // Puedes ejecutar cualquier otra lógica que necesites aquí
});

defineExpose({ data, getIngredientActiveProduct });
</script>
